...
namespace Shuttle.Esb.Msmq.Tests
{
    ...
    public class MsmqQueueFixture : BasicQueueFixture
    {
        ...
        {
            TestUnacknowledgedMessage(MsmqFixture.GetServiceCollection(), "msmq://./{0}");
        }

        [Test]
        public void Should_be_able_to_perform_simple_enqueue_and_get_message()
        {
            TestSimpleEnqueueAndGetMessage(MsmqFixture.GetServiceCollection(), "msmq://./{0}");
        }

        [Test]
        public void Should_be_able_to_release_a_message()
        {
            TestReleaseMessage(MsmqFixture.GetServiceCollection(), "msmq://./{0}");
        }
    }
}